  ...
  render() {
    ...
    return (
      <div className={styles["home-default-page"]}>
        <a href="http://github.com/supnate/rekit"><img src={require('../../images/logo.png')} className={styles["app-logo"]} alt="logo" /></a>
        ...
        <p className={styles["section-title"]}>To see how Redux works in the project, here is the demo of a simple counter:</p>
        <div className={styles["demo-count"]}>
          <button className={styles["btn-minus-one"]} onClick={counterMinusOne} disabled={count === 0}>-</button>
          <label>{count}</label>
          <button className={styles["btn-plus-one"]} onClick={counterPlusOne}>+</button>
          <button className={styles["btn-reset-counter"]} onClick={resetCounter}>Reset</button>
        </div>

        <p className={styles["section-title"]}>To see how async flow works, here is an example of fetching reddit reactjs topics:</p>
        <div className={styles["demo-reddit"]}>
          <button className={styles["btn-fetch-reddit"]} disabled={fetchRedditReactjsListPending} onClick={fetchRedditReactjsList}>
            {fetchRedditReactjsListPending ? 'Fetching...' : 'Fetch reactjs topics'}
          </button>
          {
            fetchRedditReactjsListError &&
              <div className={styles["fetch-list-error"]}>
